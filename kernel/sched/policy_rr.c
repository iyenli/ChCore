...
struct thread* rr_sched_choose_thread(void)
{
    ...
    /* LAB 4 TODO BEGIN */
    if (list_empty(&(rr_ready_queue_meta[smp_get_cpu_id()].queue_head))) {
        return &(idle_threads[smp_get_cpu_id()]);
    }

    thread = list_entry(rr_ready_queue_meta[smp_get_cpu_id()].queue_head.prev,
...
{
    /* LAB 4 TODO BEGIN */
    if (target != NULL && target->thread_ctx != NULL) {
