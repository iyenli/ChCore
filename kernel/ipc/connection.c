    ...
    struct ipc_vm_config* client_vm_config)
{
    ...
    if (conn_cap < 0) {
        kinfo("Error quit here\n");
        ret = conn_cap;
        goto out_free_obj;
    }

    server_conn_cap = cap_copy(current_cap_group, target->cap_group, conn_cap);
...
static u64 thread_migrate_to_server(struct ipc_connection* conn, u64 arg)
{
    ...
    struct server_ipc_config* target_ipc_config = (struct server_ipc_config*)(target->general_ipc_config);
...
static void ipc_send_cap_to_client(struct ipc_connection* conn, u64 cap_num)
{
    ...
    server_ipc_msg = (struct ipc_msg*)((u64)ipc_msg - conn->buf.client_user_addr
...
u64 sys_register_server(u64 callback, u64 max_client, u64 vm_config_ptr)
{
    ...
    server_ipc_config->conn_bmp = kzalloc(BITS_TO_LONGS(max_client) * sizeof(long));
