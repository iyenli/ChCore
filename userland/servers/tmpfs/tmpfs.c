    ...
    size_t size)
{
    ...
    /* LAB 5 TODO BEGIN */
    u64 target = offset + size;

    while (cur_off < target) {
        page_no = cur_off / PAGE_SIZE, page_off = cur_off % PAGE_SIZE;
        ...
        BUG_ON(page == NULL);
        to_write = MIN(target - cur_off, PAGE_SIZE - page_off);
        memcpy(page + page_off, data + (cur_off - offset), to_write);
        cur_off += to_write;
    }

    if (offset > inode->size) {
        WARN("Offet > file size, know how to handle it\n");
    }
    ...
    size_t size)
{
    ...
    /* LAB 5 TODO BEGIN */
    to_read = MIN(size, (inode->size - offset));
    while (to_read > 0) {
        page_no = cur_off / PAGE_SIZE, page_off = cur_off % PAGE_SIZE;

        size_t page_read_size = MIN(to_read, PAGE_SIZE - page_off);
        page = radix_get(&(inode->data), page_no);

        BUG_ON(page == NULL);
        memcpy(buff + cur_off - offset, page + page_off, page_read_size);
        to_read -= page_read_size;
        cur_off += page_read_size;
    }
    /* LAB 5 TODO END */
